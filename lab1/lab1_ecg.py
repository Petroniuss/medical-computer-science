...
df = pd.read_csv('ecg.data', sep=',')
ecg = df['ECG'].values[:max_samples]
print('ecg data: ')
print(ecg)
...
for r_peak in r_peaks[1:]:
    time_between_heartbeats.append((r_peak - prev_r) / 100.0)
    prev_r = r_peak
...
filtered_ecg = moving_average(ecg)
...
filtered_r_peaks, _ = find_peaks(filtered_ecg, height=.13, distance=15)
...
plt.plot(r_peaks, ecg[r_peaks], "bo")
plt.plot(filtered_p_peaks, ecg[filtered_p_peaks], "go")
plt.plot(filtered_t_peaks, ecg[filtered_t_peaks], "co")
